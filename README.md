...
“BH1JSS机婶婶测试台”是本项目在FMO网络中的实际验证服务器，由BH1JSS维护，用来测试AI语音问答、无线电知识服务和FMO语音链路接入。

测试台的AI发言标识为 `AI-BH1JSS`，与真实用户呼号区分。它可以进行普通聊天，回答无线电知识、FMO仪表盘和HAM台网点名控制台的使用问题，并按需检索NAS中的设备说明书与自建知识库。遇到CQ、呼号、字母解释法和73等标准通联内容时，会优先使用简洁的业余无线电通联方式回应。

测试台通常在北京时间7:00至24:00运行，支持整点报时、夜间休息提示、网页与语音开关、呼号统计和黑名单。它仍属于持续验证中的AI实验服务，不代表真实持证操作员，也不会替代FMO原有SAS/CA鉴权体系。

![BH1JSS机婶婶测试台AI网关管理界面](docs/images/ai-gateway-status.jpg)

> 管理界面截图为脱敏展示，只呈现页面布局和功能区域，不包含API Key、Token、密码、服务器地址或实时用户数据。
